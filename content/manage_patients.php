...
<i class="nav-icon fas fa-search"></i>Search</button>
    ...
    <div style="background-color:#fff;width:100%;height:600px;overflow:auto" id="searchContent">
         ...
         <?php
$sn='1';
$sqlGetStudent=mysqli_query($con,"SELECT *FROM patient_information WHERE status='1' ORDER BY patientID DESC LIMIT 10");
if($sqlGetStudent){
    ...
    if($sqlGetStudentRow > 0){
        ...
        while($rows=mysqli_fetch_array($sqlGetStudent)){
            ...
            <td width="10%"><a href="#" style="color:green;" onclick="ViewMedicalRecord('.$patientID.')"> <i class="nav-icon fas fa-file"></i>View File</a></td>
        </tr>';
