...
<!-- Navbar -->
  ...
  <div class="content-wrapper">
    ...
    <div style="background-color:#fff;width:100%;height:600px;overflow:auto" id="searchContent">
		...
		<div class="card-body">
              <div class="row">
                <h5 class="card-title">Drug Usage Report</h5>
              </div>
                <div class="row">
               
                  <div class="col-md-8">
                    <p class="text-center">
                      <strong>Phamacy facility Access</strong>
                    </p>
					...
					$sqlgett=mysqli_query($con,"SELECT *FROM patient_drug");
						if($sqlgett){
							$sqlgetrow=mysqli_num_rows($sqlgett);
							if($sqlgetrow > 0){
								while($Get=mysqli_fetch_array($sqlgett)){
									$student_id=$Get['patient_id'];
									$drug_id=$Get['drug_id'];
									
					$sqlgetGender=mysqli_query($con,"SELECT *FROM patient_information WHERE patientID='$student_id'");
						if($sqlgetGender){
							$sqlgetGender_row=mysqli_num_rows($sqlgetGender);
							if($sqlgetGender_row > 0){
									$Gett=mysqli_fetch_array($sqlgetGender);
									$student_id=$Gett['patientID'];
									$gender=$Gett['patientGender'];
						...
						$maleP=($pMale/$sqlgetrow) * 100;
						$femaleP=($pFEMale/$sqlgetrow) * 100;
								?>
						<table class="table" width="100%">
							<tr>
								<td><b> <div class="progress-group">
                     Male (<?php echo $maleP; ?>)
                      <span class="float-right"></span>
                      <div class="progress progress-sm">
                        <div class="progress-bar bg-success" style="width:  <?php echo $maleP.'%'; ?>"></div>
                      </div>
                    </div></b></td>
							</tr>
							<tr>
								<td><b> <div class="progress-group">
                     Female (<?php echo $femaleP; ?>)
                      <span class="float-right"></span>
                      <div class="progress progress-sm">
                        <div class="progress-bar bg-success" style="width: <?php echo $femaleP.'%'; ?>"></div>
                      </div>
                    </div></b></td>
							</tr>
						</table>
                  ...
                  <div class="col-md-4">
                    <p class="text-center">
                      <strong>Drug Usage Information</strong>
                    </p>
						<a href="#"><?php echo $mostUsed; ?></a>
                    <!-- /.progress-group -->
                  </div>
                  <!-- /.col -->
                </div>
